...
class EmrpcServer{
  ...
  function getItemNumber($sid,$idmenulists){
	...
	if($query->num_rows() == 0)
		return json_encode(array(array('number'=>0)));
	else
		return json_encode($query->result());	
	
  }
  
  function addToOrder($sid,$idmenulists,$number){
	...
	if($isUpdate){
		if($number=='0')
			...
			$data = array('number'=>$number,'timestamp'=>date("Y-m-d H:i:s"));
			$this->db->where(array('idOrders'=>$idOrders));
			$this->db->update('orders',$data);
		}
	}
	else{		
		$data = array('fk_sid'=>$sid,'fk_idMenulists'=>$idmenulists,'number'=>$number,'timestamp'=>date("Y-m-d H:i:s"));
