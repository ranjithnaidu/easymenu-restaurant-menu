...
class Bar_model extends CI_Model{
	...
	function getTicketBySid($sid){
		$this->db->select('fk_idMenulists,number,bqueue.status,idOrders,bqueue.fk_sid');
		$this->db->from('bqueue');
		...
		foreach($fixed as $row){
			$returnData['f'][] = array('idMenulists'=>$row->idMenulists,'label'=>$row->label,'number'=>$idMenulists[$row->idMenulists]['nr'],'status'=>$idMenulists[$row->idMenulists]['status'],'idOrders'=>$idMenulists[$row->idMenulists]['idOrders']); 
		}
		
		foreach($carte as $row){
			$returnData['c'][] = array('idMenulists'=>$row->idMenulists,'label'=>$row->label,'number'=>$idMenulists[$row->idMenulists]['nr'],'status'=>$idMenulists[$row->idMenulists]['status'],'idOrders'=>$idMenulists[$row->idMenulists]['idOrders']); 
		}
