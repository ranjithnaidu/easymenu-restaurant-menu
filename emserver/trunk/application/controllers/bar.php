...
class Bar extends CI_Controller{
	...
	function getOrderQueueContent(){
		...
		foreach($data as $row){
			echo "<div id='tableclear' class='klist'><img style='position:relative;top:-5px;left:-5px;' src=\"".base_url()."/img/bell.png\">$row->tableName<br><button id=\"clear-".$row->sid."\" style='font-size:12px;'>Clear call</button></div>";
		}
	}
